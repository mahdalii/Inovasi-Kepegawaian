...
import config
from app import SCHEMA_SQL, init_db, create_app, gen_nomor, save_pengajuan, is_late, can_transition, validate_form, VALID_UPTD
...
def test_init_db_membuat_tabel_cuti():
    with tempfile.TemporaryDirectory() as d:
        ...
        conn.close()
    assert cols == {"id", "nomor", "nama", "uptd", "jenis", "tgl_mulai",
...
def test_update_status_menolak_mundur():
    with tempfile.TemporaryDirectory() as d:
        ...
        save_pengajuan(conn, "Budi", "batam_centre", "sakit", "2026-08-13",
...
def test_validate_form_menolak_data_tidak_lengkap():
    errors = validate_form("", "haha", "haha", "2026-08-13", "2026-08-14", False, True)
...
def test_validate_form_menolak_uptd_kosong_dan_valid():
    ...
    assert any("wajib" in e for e in errors)
    assert validate_form("Dewi", "batuaji", "tahunan", "2026-08-13",
                         "2026-08-14", True, True) == []

def test_validate_form_menolak_tanggal_terbalik_dan_file_besar():
    errors = validate_form("A", "bintan", "tahunan", "2026-08-15", "2026-08-14", True, False)
...
def test_gen_nomor_berurutan():
    ...
    conn.execute(
        ...
        "VALUES ('CUT-2026-0001','X','lingga','sakit','2026-08-13','2026-08-14','b.jpg','Baru','','2026-08-13')"
    )
    conn.commit()
...
def _post_berkas(client, nama_file, ukuran):
    return client.post("/", data={
        "nama": "Dewi", "uptd": "batuaji", "jenis": "sakit",
